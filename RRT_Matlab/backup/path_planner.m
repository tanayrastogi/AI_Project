function Vrep_pos = path_planner()
close all;
clc;
...
for i = 1:1:max_nodes
    ...
    % Check if the point is inside the boundry and not inside any obstacle
    if(bounded(q_rand, dims, ob1, ob2, ob3, ob4, ob5, ob6))   
        plot(q_rand(1), q_rand(2), 'x');
        ...
        % Move towards the point according to the motion model
%       q_new.coord = motion(q_rand, q_near.coord, val, map_data.start_vel);
        q_new.coord = q_rand;            
        
        % Check if the line between the random point and the nearest does
        % not intersect to any obstacle
        x = [q_rand(1), q_near.coord(1)];
        y = [q_rand(2), q_near.coord(2)];
        [~,~,~, flag] = intersection(x, y, ob1, ob2, ob3, ob4, ob5, ob6);
        if(flag)
            figure(1)
